# Anomaly Threat Intelligence: Reproduction Report
...
## 2. Problem and data
The task is anomaly detection. The reference dataset/theme is Credit-card-fraud style synthetic data. For offline verification, this submission uses a deterministic built-in or synthetic equivalent when network data is not required for the concept.
...
## 5. Evaluation
The primary reported measure is **ROC-AUC**. Exact generated values are kept in `artifacts/metrics.json` rather than hard-coded in this report. This prevents the written result from becoming stale after reruns.
